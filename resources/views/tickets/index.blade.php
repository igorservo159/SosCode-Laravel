<x-app-layout>
    <div class="max-w-2xl mx-auto p-4 sm:p-6 lg:p-8">
        <form method="POST" action="{{ route('tickets.store') }}">
            @csrf
            <textarea
                name="message"
                placeholder="{{ __('Escreva aqui seu ticket') }}"
                class="block w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm"
            >{{ old('message') }}</textarea>
            <x-input-error :messages="$errors->get('message')" class="mt-2" />

            <label for="category">Categoria:</label>
            <select name="category" id="category" required>
                <option value="Desenvolvimento Web">Desenvolvimento Web</option>
                <option value="Programação Embarcada">Programação Embarcada</option>
                <option value="Inteligência Artificial">Inteligência Artificial</option>
                <option value="Banco de Dados">Banco de Dados</option>
                <option value="Outros">Outros</option>
            </select>
            <x-input-error :messages="$errors->get('category')" class="mt-2" />
            <x-primary-button class="mt-4">{{ __('Ticket') }}</x-primary-button>
        </form>

        <div class="mt-6 bg-white shadow-sm rounded-lg divide-y">
            @foreach ($tickets as $ticket)
                <div class="p-6 flex space-x-2">
                    <div class="flex-1">
                        <div class="flex justify-between items-center">
                            <div>
                                <span class="text-gray-800">{{ $ticket->user->name }}</span>
                                <small class="ml-2 text-sm text-gray-600">{{ $ticket->created_at->format('j M Y, g:i a') }}</small>
                                @unless ($ticket->created_at->eq($ticket->updated_at))
                                    <small class="text-sm text-gray-600"> &middot; {{ __('edited') }}</small>
                                @endunless
                            </div>
                            @if ($ticket->user->is(auth()->user()))
                                <x-dropdown>
                                    <x-slot name="trigger">
                                        <button>
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-gray-400" viewBox="0 0 20 20" fill="currentColor">
                                                <path d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z" />
                                            </svg>
                                        </button>
                                    </x-slot>
                                    <x-slot name="content">
                                        <x-dropdown-link :href="route('tickets.edit', $ticket)">
                                            {{ __('Edit') }}
                                        </x-dropdown-link>
                                        <form method="POST" action="{{ route('tickets.destroy', $ticket) }}">
                                            @csrf
                                            @method('delete')
                                            <x-dropdown-link :href="route('tickets.destroy', $ticket)" onclick="event.preventDefault(); this.closest('form').submit();">
                                                {{ __('Delete') }}
                                            </x-dropdown-link>
                                        </form>
                                    </x-slot>
                                </x-dropdown>
                            @endif
                        </div>
                        <div>
                            <span class="text-gray-800">Categoria: {{ $ticket->category }}</span>
                        </div>
                        <p class="mt-4 text-lg text-gray-900">{{ $ticket->message }}</p>
                    </div>
                </div>
            @endforeach
        </div>
    </div>
</x-app-layout>